...
- **The canvas background is a fixed flat colour, not derived from the product photo**, so pack photos with differing studio backgrounds (grey, off-white, warm-tinted) don't produce a visibly inconsistent creative. Since the pack photo itself still carries its own background unless removed, the review form has an optional **"Remove image background before generating"** step that calls a local Rembg server (`rembg-server.py`, see below) to produce a transparent-background PNG that blends seamlessly onto the fixed canvas.
...
`rembg-server.py` runs a small local Flask + Rembg server (`python3 -m venv rembg-env && source rembg-env/bin/activate && pip install rembg flask requests pillow && python3 rembg-server.py`, listening on `127.0.0.1:5000`) that `/api/remove-background` calls. It's a local, optional, non-hard-stop enhancement — if the server isn't running, background removal fails gracefully with an inline message and you can still generate with the original photo. It doesn't run on Vercel; it's a dev-time tool for producing cleaner source images.
...
- **Rembg's cutout isn't pixel-perfect.** On the real product photos tested, background removal correctly dropped the studio backdrop but sometimes left a faint shadow/reflection blob near the product base. It's a clear net improvement over the original background (which otherwise shows as a visible rectangle against the creative's fixed canvas colour), but it's a local, best-effort tool, not a guaranteed-clean cutout — worth a visual check before using a processed image.
